...
graficar_campo  <- function( campo )
{
  ...
  densidad_A  <- density( dataset[ foto_mes==202103 , get(campo) ], kernel="gaussian", na.rm=TRUE )
  
  densidad_B  <- density( dataset[ foto_mes==202105 , get(campo) ], kernel="gaussian", na.rm=TRUE )
...
dataset  <- fread("./labo/lisandro/competencia2/FE/dataset_fe_c2_rankeado_pormes_nobinaria.csv")
...
numericas <- colnames(numericas)
numericas <- c(numericas, 'clase_ternaria')
...
campos_buenos  <-  setdiff(  campos_buenos,  c( "foto_mes","clase_ternaria" ) )
